...
// The active buzzer should buzz when the 1D7S displays either '4' in AM mode, or 'E' in FM mode.
//for this, we will create more state transitions that will handle the buzzer turning on and off when those values are displayed.

switch (state) {
    case AM_mode:
        adc_val = ADC_read(0);   // read A0
        display = map(adc_val, 0, 1023, 0x00,0x09); //9 in hex is 0x09
        //here we'll check if its 4, then turn on buzzer
        outNum(display); // display  digit
        if (display == 0x04){
            PORTD = SetBit(PORTD,2,1); //turn on buzzer
        }
        else{
            PORTD = SetBit(PORTD,2,0); //turn off buzzer
        }
        break;
    case FM_mode:
        adc_val = ADC_read(0);   // read A0
        display = map(adc_val, 0, 1023, 0x0A, 0x0F ); // A and F in hex is 0x0A and 0x0F
        //here we'll check if its E, then turn on buzzer
        outNum(display); // display  digit
        if(display == 0x0E){
            PORTD = SetBit(PORTD,2,1); //turn on buzzer
        }
...
DDRD |= (1<<PD7)|(1<<PD6) |(1<<PD2); //PD2 is buzzer output
